...
async def process_document(db: AsyncSession, document_id: str) -> None:
    ...
    try:
        ...
        if file_ext == ".pdf":
            chunks_to_create = await _process_pdf(file_path, document)
        elif file_ext in [".xlsx", ".xls"]:
            chunks_to_create = await _process_excel(file_path, document)
        elif file_ext in [".png", ".jpg", ".jpeg"]:
            chunks_to_create = await _process_image(file_path, document)
        else:
...
async def _process_pdf(file_path: Path, document: Document) -> list[dict]:
...
async def _process_excel(file_path: Path, document: Document) -> list[dict]:
...
async def _process_image(file_path: Path, document: Document) -> list[dict]:
